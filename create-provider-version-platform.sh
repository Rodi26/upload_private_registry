...
[[ -z "$ZIP_FILE" ]] && echo "Please Set ZIP_FILE to the file containing the SHASUMS SIG" && exit 1
[[ -z "$FILENAME" ]] && echo "Please Set FILENAME to the file containing the SHASUMS SIG" && exit 1

registry_name="private"
provider_namespace="${ORGANIZATION_NAME}"
zip_shasum=$(shasum -a 256 ${ZIP_FILE} | head -c 64)

curl -s -X POST --location "https://${HOST}/api/v2/organizations/${ORGANIZATION_NAME}/registry-providers/${registry_name}/${provider_namespace}/${PROVIDER_NAME}/versions/${PROVIDER_VERSION}/platforms" \
